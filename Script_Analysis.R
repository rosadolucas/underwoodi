...
ggnative <- ggplot(perf_all_RR, aes(x = temp, y = speed_mean, colour = "#509AF9")) +
  ...
  geom_vline(xintercept = 32.24, linewidth = 1.25, color = '#00CD00', alpha = 1, linetype = 'dotdash')+
...
ggneonative <- ggplot(perf_all_AM, aes(x = temp, y = speed_mean, colour = "#A86D00")) +
  ...
  geom_vline(xintercept = 32.70, linewidth = 1.25, color = '#00CD00', alpha = .5, linetype = 'dotdash')+
...
# Calculate the 80th percentile of the predicted values for sprint performance
...
summarySE(selected_data_AM, groupvars = 'distribution', measurevar = 'temp') # we used the mean of the gradient B80 as Topt since these values represent the better association with the curve plotted above. 

t.test(selected_data_AM$temp, selected_data_RR$temp)
